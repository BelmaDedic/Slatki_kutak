...
const FindByStatusSpecialAdmin = ({ passSetSpecialOrder }) => {
  ...
  const fetchData = async () => {
    if (status === "") {
      const data = await fetch("http://localhost:3000/Sweet/SpecialOrders");
      const specialOrder2 = await data.json();
      setSpecialOrder(specialOrder2);
      passSetSpecialOrder(specialOrder2);
    } else {
      const data = await fetch("http://localhost:3000/Sweet/findByStatusSpecial/" + status);
      const specialOrder2 = await data.json();
      setSpecialOrder(specialOrder2);
      passSetSpecialOrder(specialOrder2);
    }
...
export default FindByStatusSpecialAdmin;